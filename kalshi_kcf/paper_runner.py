...
from strategy import scan_kcf, KCFSignal, TradeDetector
...
class PaperRunner:
    ...
    def start(self):
        ...
        self.mkt.start()
        self._running = True
        self._say('KCF paper runner started')
    ...
    def stop(self):
        ...
        self._say('KCF paper runner stopped')

    def run_cycle(self) -> dict:
        self.cycle += 1
        spot = self.spot.price
        rv = None  # KCF doesn't need vol regime
        ...
        signals = scan_kcf(self.detector, markets, bankroll_avail=self.portfolio.cash, open_market_tickers=open_tickers)
        ...
        for sig in signals:
            ...
            if pos:
                cycle_payload['opened'].append({
                    ...
                    'trigger': sig.trigger_type, 'qty': sig.qty,
                    'price': sig.price, 'trade_qty': sig.trade_qty,
                    'rolling_p85': sig.rolling_p85,
                    'expected_edge': round(sig.expected_edge, 4),
                    'expected_winp': round(sig.expected_winp, 3),
                })
                self._say(f'  OPEN KCF {sig.trigger_type} {sig.market_ticker} {sig.side} '
                           f'qty={sig.qty} @${sig.price:.3f} '
                           f'(detected trade_qty={sig.trade_qty} vs p85={sig.rolling_p85}) '
