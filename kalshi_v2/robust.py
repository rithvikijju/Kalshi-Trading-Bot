...
def build_ambiguity_set(btc_1m, horizon_min: int, n_bootstrap: int = 16,
    ...
    of `log_ret`. Returns are bootstrapped (not prices) and a synthetic
    close series is reconstructed from cumulative returns; this preserves
    intraday autocorrelation within each block while avoiding artificial
    price jumps at block seams.

    Args:
        btc_1m: minute bars with column `log_ret` and `close`. Rolling
                features are recomputed per synthetic series.
        horizon_min: forward window for each measure
        n_bootstrap: |P| — number of measures (default 16, range 10-20)
        block_min: block size in minutes (default 1440 = 1 day, preserves
                   intraday vol-clustering and overnight effects)
    """
    import pandas as pd
    ...
    log_ret_full = btc_1m["log_ret"].to_numpy()
    log_ret = log_ret_full[np.isfinite(log_ret_full)]
    n_lr = len(log_ret)
    if n_lr < block_min * 2:
        return []

    seed_close = float(btc_1m["close"].iloc[0])
    af = 60 * 24 * 365
    measures = []

    for k in range(n_bootstrap):
        n_blocks = max(50, n_lr // block_min)
        block_starts = rng.integers(0, n_lr - block_min, size=n_blocks)
        synthetic_lr = np.concatenate(
            [log_ret[s:s + block_min] for s in block_starts])
        synthetic_close = seed_close * np.exp(np.cumsum(synthetic_lr))
        synthetic = pd.DataFrame({
            "close":   synthetic_close,
            "log_ret": synthetic_lr,
        })
        synthetic["rv_60m"]    = synthetic["log_ret"].rolling(60).std() * np.sqrt(af)
        synthetic["rkurt_60m"] = synthetic["log_ret"].rolling(60).kurt()
        bank = build_empirical_bank(synthetic, horizon_min,
