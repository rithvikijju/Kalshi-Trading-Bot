#!/usr/bin/env python3
"""Paper-shadow runner for BTC1H high_conf_80_entry70_no_chase.

This is a forward-validation harness, not a promoted live strategy.  It uses
the same websocket execution engine as the BTC1H live bot, but forces paper
mode, one-contract sizing, and a separate database so post-freeze decisions can
be audited without affecting capital.
"""
...
STRATEGY = "high_conf_80_entry70_no_chase"
LOCKED_ARGS = {
    "--signal-strategy",
    "--sizing-policy",
    "--db-path",
...
os.environ["BTC_1HR_EXECUTOR_NAME"] = "btc_1hr_high_conf80_entry70_no_chase_shadow"
...
def build_forward_args(user_args: list[str]) -> list[str]:
    ...
    defaults.extend(
        [
            "--db-path",
            str(Path.home() / ".btc_kalshi_bot" / "btc_1hr_high_conf80_entry70_no_chase_shadow.db"),
        ]
    )
    defaults.extend(
        [
            "--capture-db-path",
            str(Path.home() / ".btc_kalshi_bot" / "btc_1hr_high_conf80_entry70_no_chase_shadow_capture.duckdb"),
        ]
    )
