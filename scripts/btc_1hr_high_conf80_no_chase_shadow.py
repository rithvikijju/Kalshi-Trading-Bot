#!/usr/bin/env python3
"""Paper-shadow runner for the secondary BTC1H high_conf_80_no_chase candidate."""
...
STRATEGY = "high_conf_80_no_chase"
LOCKED_ARGS = {
    "--signal-strategy",
    "--sizing-policy",
    "--db-path",
...
os.environ["BTC_1HR_EXECUTOR_NAME"] = "btc_1hr_high_conf80_no_chase_shadow"
...
def build_forward_args(user_args: list[str]) -> list[str]:
    ...
    defaults.extend(["--db-path", str(Path.home() / ".btc_kalshi_bot" / "btc_1hr_high_conf80_no_chase_shadow.db")])
    defaults.extend(
        [
            "--capture-db-path",
            str(Path.home() / ".btc_kalshi_bot" / "btc_1hr_high_conf80_no_chase_shadow_capture.duckdb"),
        ]
    )
