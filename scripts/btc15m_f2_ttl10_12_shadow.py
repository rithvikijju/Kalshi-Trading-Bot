#!/usr/bin/env python3
"""Paper BTC15M F2 TTL 10-12m shadow.

This is a forward-validation wrapper for the frozen candidate:
    ...
    spread <= 2c
    entry 2c-55c
    visible top ask quantity >= 50

It never submits orders because it runs `btc15m_lowdd_live.py` in paper mode
with a separate mock bankroll/trade DB. Use it to collect future
orderbook/decision rows plus a forward paper ledger.
"""
...
def main() -> None:
    data_dir = Path(os.path.expanduser("~/.btc_kalshi_bot"))
    ...
    os.environ.setdefault("BTC15M_H02_ENTRY_MAX", "0.55")
    os.environ.setdefault("BTC15M_H02_MIN_SIDE_PROB", "0.60")
    os.environ.setdefault("BTC15M_H02_MIN_VISIBLE_QTY", "50")
    ...
    sys.argv = [
        ...
        "h02",
        "--capture-db-path",
        str(data_dir / "btc15m_f2_ttl10_12_shadow_capture.duckdb"),
        "--trade-db-path",
        str(data_dir / "btc15m_f2_ttl10_12_shadow_trades.db"),
        "--shadow-bankroll",
        "100",
        "--refresh-sec",
        "10",
