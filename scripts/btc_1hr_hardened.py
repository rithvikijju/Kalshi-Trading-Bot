...
PAPER_DB_PATH = str(PAPER_DB_DIR / "hardened_trades.db")
...
_LOG_FILE = _LOG_DIR / f"btc_1hr_hardened_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
def tradeable_signals(edges):
    ...
    # Cumulative (-T) markets need higher edge (29.8% win rate overnight vs 51% for buckets)
    is_cumulative = edges["ticker"].str.contains("-T", na=False)
    cum_min_edge = max(cfg["min_edge_cents"], 10.0)
    edge_threshold = np.where(is_cumulative, cum_min_edge, cfg["min_edge_cents"])
