...
PAPER_DB_PATH = str(PAPER_DB_DIR / "paper_draft_trades.db")

# ── Logging setup ──────────────────────────────────────────────────────────
_LOG_DIR = Path(_PROJECT_ROOT) / "logs"
_LOG_DIR.mkdir(exist_ok=True)
_LOG_FILE = _LOG_DIR / f"btc_1hr_paper_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
def tradeable_signals(edges):
    ...
    if len(passed) == 0 and len(edges) > 0:
        ...
        reasons = []
        if top["net_edge_cents"] < cfg["min_edge_cents"]: reasons.append(f"edge {top['net_edge_cents']:.1f}<{cfg['min_edge_cents']}")
