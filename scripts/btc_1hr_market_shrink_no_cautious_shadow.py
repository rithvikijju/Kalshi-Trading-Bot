#!/usr/bin/env python3
"""Paper-shadow runner for the market_shrink_no_cautious BTC 1-hour strategy.

This reuses btc_1hr_research_live.py's websocket/orderbook, capture,
event-lock, and shadow-bankroll logic. Defaults are paper-only with a $1000
mock bankroll and separate DB paths.
"""
...
os.environ.setdefault("BTC_1HR_EXECUTOR_NAME", "btc_1hr_market_shrink_no_cautious_shadow")
os.environ.setdefault("BTC_1HR_SIGNAL_STRATEGY", "market_shrink_no_cautious")
...
def main() -> None:
    ...
    if not has_arg("--signal-strategy", user_args):
        defaults.extend(["--signal-strategy", "market_shrink_no_cautious"])
    ...
    if not has_arg("--db-path", user_args):
        defaults.extend(["--db-path", str(Path(CFG["db_dir"]).expanduser() / "market_shrink_no_cautious_shadow_trades.db")])
    if not has_arg("--capture-db-path", user_args):
        defaults.extend(
            [
                "--capture-db-path",
                str(Path(CFG["db_dir"]).expanduser() / "market_shrink_no_cautious_shadow_capture.duckdb"),
            ]
        )

    live.set_signal_strategy("market_shrink_no_cautious")
