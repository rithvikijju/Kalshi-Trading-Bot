#!/usr/bin/env python3
"""Paper-shadow runner for BTC1H high_conf_80_entry59_70_no_chase.

This is a forward-validation harness, not a promoted live strategy.  It mirrors
the websocket execution engine used by the live BTC1H bot, but forces paper
mode, one-contract sizing, and separate persistence so the new entry floor can
...
STRATEGY = "high_conf_80_entry59_70_no_chase"
LOCKED_ARGS = {
    "--signal-strategy",
    "--sizing-policy",
    "--db-path",
...
os.environ["BTC_1HR_EXECUTOR_NAME"] = "btc_1hr_high_conf80_entry59_70_no_chase_shadow"
...
def build_forward_args(user_args: list[str]) -> list[str]:
    ...
    defaults.extend(
        [
            "--db-path",
            str(Path.home() / ".btc_kalshi_bot" / "btc_1hr_high_conf80_entry59_70_no_chase_shadow.db"),
        ]
    )
    defaults.extend(
        [
            "--capture-db-path",
            str(Path.home() / ".btc_kalshi_bot" / "btc_1hr_high_conf80_entry59_70_no_chase_shadow_capture.duckdb"),
        ]
    )
