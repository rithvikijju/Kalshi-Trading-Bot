...
def test_verdict_distinguishes_filterable_book_rows_from_hard_failure():
    ...
    coinbase = pd.DataFrame(
        [
            ...
            {"metric": "raw_coinbase_schema_fields_present", "value": True},
            {"metric": "coinbase_ticker_source", "value": "raw_sidecar_schema"},
        ]
    )
...
def test_verdict_rejects_synthetic_coinbase_without_manifest():
    quote = pd.DataFrame(
        [
            {"metric": "rows", "value": 100},
            {"metric": "null_received_at_utc", "value": 0},
            ...
            {"metric": "negative_qty", "value": 0},
            {"metric": "implausible_btc_spot", "value": 0},
            {"metric": "valid_book_rows", "value": 100},
            {"metric": "valid_book_rate", "value": 1.0},
        ]
    )
    gaps = pd.DataFrame([{"gaps_over_threshold": 0}])
    coinbase = pd.DataFrame(
        [
            {"metric": "synthetic_coinbase_from_top", "value": True},
            {"metric": "raw_coinbase_schema_fields_present", "value": False},
            {"metric": "coinbase_ticker_source", "value": "synthetic_or_unknown_schema"},
        ]
    )

    out = verdict(quote, gaps, coinbase, manifest={}, compare_df=pd.DataFrame()).iloc[0]

    assert not bool(out["promotion_grade_coinbase_ticks"])
    assert "not promotion-grade BTC tick-age evidence" in out["notes"]
